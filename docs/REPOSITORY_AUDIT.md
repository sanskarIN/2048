...
The audit fails when required project, open-source, release, support, security, build, or workflow files are missing or empty. This includes the primary README, changelog, roadmap, license, contribution/security/support files, release-qualification files, build handbook, and permanent CI workflows.
...
Known one-shot Phase 30 helper paths are forbidden from remaining in the final repository. If one is accidentally reintroduced, the audit fails instead of allowing temporary release-maintenance machinery to become permanent dead configuration.

### Local Markdown links

The audit scans top-level Markdown plus Markdown under `docs/` and `.github/` and validates local file/directory destinations.
