...
void main() {
  ...
  test('custom session persists without creating built-in mode records', () async {
  ...
  test('starting a built-in game clears custom identity and records mode data', () async {
    final controller = AppController(store: LocalStore());
    await controller.initialize();
    await controller.newGame(
    ...
    await controller.newGame(GameConfig.preset(GameMode.target, target: 4096));
